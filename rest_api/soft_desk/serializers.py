...
class UserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not (first_name and last_name):
            raise serializers.ValidationError("you must set a not empty string value for first_name and last_name")
        return data
...
class IssueSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Issue
        fields = ['issue_id', 'title', 'desc', 'tag', 'priority', 'project_id', 'status', 'author_user_id',
    ...
    def validate(self, data):
        ...
        if not the_assignee_user.is_contributor(the_project) and not (the_assignee_user == the_project.author_user):
            raise serializers.ValidationError(
                f"assignee_user ({the_assignee_user.user_id} - {the_assignee_user}) has no permission (contributor or owner) in project ({the_project.project_id} - {the_project})")
        return data
